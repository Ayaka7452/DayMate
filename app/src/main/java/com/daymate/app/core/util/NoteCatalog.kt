...
object NoteCatalog {
     ...
     *
     * ⚠️ 定性口径：体外射精**不得写成「避孕方式 / 避孕法」**（v1.9.3 用户指出）。
     * 它确实在分类上属传统避孕法，但是失败率垫底的一档（CDC：理想使用 4% / 实际使用 22%）；
     * 这条提示出现的场景恰恰是**用户刚勾了「有保护措施」**，此时给一个中性分类描述，
     * 等于替这个做法背书、让用户以为已经做够了保护。一律写成「并不算有效的保护措施」，
     * 并保留原始诉求里的四项：怀孕风险 / 留意身体状况 / 及时评估 / 无法阻断疾病传播。
     */
