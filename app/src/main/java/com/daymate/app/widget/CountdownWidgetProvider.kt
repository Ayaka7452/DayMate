 ...
 * - 生命周期全程写 AppLogger 诊断日志（设置页可开关，仅本机）。
 */
class CountdownWidgetProvider : AppWidgetProvider() {

    override fun onUpdate(context: Context, manager: AppWidgetManager, appWidgetIds: IntArray) {
        AppLogger.log(context, "Wide", "onUpdate ids=${appWidgetIds.joinToString()}")
        val pending = goAsync()
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                AppLogger.log(context, "Wide", "onUpdate 渲染完成")
            } catch (t: Throwable) {
                AppLogger.logError(context, "Wide", "onUpdate 渲染异常", t)
            } finally {
                pending.finish()
            }
        }
    ...
    override fun onEnabled(context: Context) {
        AppLogger.log(context, "Wide", "onEnabled（桌面上出现第一个宽版小组件）")
    ...
    override fun onDisabled(context: Context) {
        AppLogger.log(context, "Wide", "onDisabled（桌面上最后一个宽版小组件被移除）")
    ...
    override fun onAppWidgetOptionsChanged(
        ...
        AppLogger.log(context, "Wide", "onAppWidgetOptionsChanged id=$appWidgetId")
        super.onAppWidgetOptionsChanged(context, manager, appWidgetId, newOptions)
    }

    override fun onReceive(context: Context, intent: Intent) {
        AppLogger.log(context, "Wide", "onReceive action=${intent.action}")
        when (intent.action) {
